...
field.events.afterUpdate.listen(() => {
    phoneNumber.value = parsePhoneNumber(field.value ? field.value : "");
})
    ...
    <FieldWrapper :field="field" :no-label="noLabel">
        ...
        <template #="{ id }">
            <input :id="id" class="form-control" :class="{ 'is-invalid': field.hasErrors }"
                :value="field.value?field.value.replace(`+${countryCallingCode}`, ''):''" v-bind="$attrs" @input="update" @focus="touch"
