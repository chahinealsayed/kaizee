    ...
    <div class="count" v-if="total > 0">
      {{
        translate(
          ...
          perPage * currentPage <= total ? perPage * currentPage : total, total) }} </div>
        <nav aria-label="Page navigation" v-if="items.length > 1" class="pages">
          <ul class="pagination pagination-sm">
            <li class="page-item" :class="{ disabled: currentPage === 1 }">
              <a class="page-link" href="#" tabindex="-1" aria-disabled="true" @click.prevent="goTo(1)">
                <Icon :path="mdiChevronDoubleLeft" />
              </a>
            </li>
            <li class="page-item" :class="{ disabled: currentPage === 1 }">
              <a class="page-link" href="#" tabindex="-1" aria-disabled="true" @click.prevent="goTo(currentPage - 1)">
                <Icon :path="mdiChevronLeft" />
              </a>
            </li>
            <li class="page-item" v-for="p in items" :key="p" :class="{ active: p === currentPage }">
            ...
            <li class="page-item" :class="{ disabled: currentPage === pagesNb }">
              <a class="page-link" href="#" tabindex="-1" aria-disabled="true" @click.prevent="goTo(pagesNb)">
        ...
        <div class="per-page">
          <Dropdown v-if="total > 0" ref="dropDown" :title="`${perPage} ${translate('rows')}`" auto-close>
            <template v-for="(size, i) in sizes" :key="size">
              <DropdownItem v-if="i === 0 || (sizes.at(i - 1) ?? 0) <= total" @click.prevent="changeSize(size)">{{ size }}
                {{ translate("rows") }}</DropdownItem>
            </template>
          </Dropdown>
        </div>
    </div>
